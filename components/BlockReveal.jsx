...
export default function BlockReveal({
  ...
  useGSAP(
    () => {
      ...
      let elements = [];
      if (containerRef.current.hasAttribute("data-block-wrapper")) {
      ...
      const createReveal = (block, line, index) => {
      ...
      if (animateOnScroll) {
        blocks.current.forEach((block, index) => {
          const tl = createReveal(block, lines.current[index], index);
          tl.pause();
        ...
        blocks.current.forEach((block, index) => {
          createReveal(block, lines.current[index], index);
        });
      }
  ...
  return (
    <div ref={containerRef} data-block-wrapper="true">
      {children}
    </div>
  );
}
